...
def parse_args():
    ...
    parser.add_argument('--curriculum-id', type=int, help='Curriculum ID for lesson generation')
...
def setup_lesson_instruction(args):
    ...
    if not args.curriculum_id:
        raise ValueError("--curriculum-id is required for lesson mode")

    db = DatabaseManager()
    curriculum = db.get_curriculum(args.curriculum_id)
    if not curriculum:
        raise ValueError(f"Curriculum {args.curriculum_id} not found")

    # Parse curriculum data from the database
    ...
    return generate_lesson(lesson_prompt, args.curriculum_id, args.week)

def setup_daily_lesson(args):
    """Setup and generate daily lesson using specified day"""
    if not args.curriculum_id:
        raise ValueError("--curriculum-id is required for daily mode")

    db = DatabaseManager()
    lesson = db.get_lesson(args.curriculum_id, args.week)
    if not lesson:
        raise ValueError(f"Lesson not found for curriculum {args.curriculum_id}, week {args.week}")

    theme = lesson["theme"]
    lessons = lesson["daily_lessons"]
    ...
    return generate_daily_lesson(lesson_prompt, args.curriculum_id, args.week, args.day)
